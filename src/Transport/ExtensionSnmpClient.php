...
final class ExtensionSnmpClient implements SnmpClient
{
    ...
    public function __construct(
        ...
        $this->snmp                     = new SNMP($snmpVersion, $host, $community, $timeout, $retry);
    ...
    {
        try {
            return $this->processOutput($this->snmp->get($oids));
        } catch (Throwable $throwable) {
            throw $this->processException($throwable);
        }
    }
    ...
    {
        try {
            return $this->processOutput($this->snmp->getnext($oids));
        } catch (Throwable $throwable) {
            throw $this->processException($throwable);
        }
    }
    ...
    {
        try {
            return $this->processOutput($this->snmp->walk($oid, false, 50));
        } catch (Throwable $throwable) {
            throw $this->processException($throwable);
        }
    }
    ...
    private function processException(Throwable $throwable) : Throwable
    {
        ...
        return GeneralException::new($throwable->getMessage(), $throwable);
    }
}
